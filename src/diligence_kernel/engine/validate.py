...
def validate_cell(
    ...
    *,
    ...
    if not text:
        return [Violation("CELL_EMPTY", "The cell is empty; every cell must state a value or a fallback state.")]
    ...
    if not fallback and native_type not in {"Verbatim", "Free Response"} and CITATION_RE.search(text):
        violations.append(Violation(
            "CITATION_IN_CELL",
